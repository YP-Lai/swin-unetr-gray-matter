# -*- coding: utf-8 -*-
"""
This script provides the training pipeline for the model.    
Several transforms were found to cause artifacts or inconsistencies in cortical segmentation tasks:
1. RandFlip
   - Issue: Caused inconsistencies in cortical segmentation 
            (e.g., hemisphere flipping, unstable boundaries).
   - Fix: Removed to ensure accurate cortex segmentation.

2. ConvertToMultiChannelBasedOnBratsClassesd
   - Issue: Not suitable for cortex segmentation; 
            introduced label-related inconsistencies during training.
   - Fix: Removed from the pipeline.
After removing the above transforms, the trained model no longer produces white lines or segmentation instability in cortex-related tasks.
@author: user
"""
